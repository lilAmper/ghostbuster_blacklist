...
exports.getServerPlayers = exports.getServerInfo = void 0;
...
var safeRequest = function (ipaddress, port, opcode, safeResponse) {
    if (validateIPAddress(ipaddress)) {
        ...
        dns_1["default"].lookup(ipaddress, function (err, result) {
            if (err)
                safeResponse.apply(ipaddress, [err, result]);
            request(ipaddress, port, opcode, function (error, response) { safeResponse.apply(ipaddress, [error, response]); });
        });
    }
};
    ...
    socket.on('message', function (message) {
        ...
        if (message.length < 11) {
            ...
            if (opcode === 'i') {
                ...
                var propertiescount = message.readUInt16LE(offset);
                offset += 2;
                ...
                object = [];
                var playerslist = [], playercount = message.readUInt16LE(offset);
                offset += 2;
...
var getServerInfo = function (serverip, serverport, reply) {
    safeRequest.call(1, serverip, serverport, "i", function (error, response) {
        if (error)
            return reply.apply(serverip, [true, response]);
        var serverinfo = response;
        safeRequest.call(1, serverip, serverport, "r", function (error, response) {
            if (error)
                return reply.apply(serverip, [true, response]);
...
var getServerPlayers = function (serverip, serverport, reply) {
    getServerOnline.call(1, serverip, serverport, function (error, response) {
        if (!isFinite(response) || response > 100)
            return reply.apply(serverip, [true, "[error] more 100 players on server"]);
        safeRequest.call(1, serverip, serverport, "c", function (error, response) {
            if (error)
                return reply.apply(serverip, [true, response]);
            return reply.apply(serverip, [false, response]);
        });
...
var getServerPlayersDetailed = function (serverip, serverport, reply) {
    getServerOnline.call(1, serverip, serverport, function (error, response) {
        if (!isFinite(response) || response > 100)
            return reply.apply(serverip, [true, "[error] more 100 players on server"]);
        safeRequest.call(1, serverip, serverport, "d", function (error, response) {
            if (error)
                return reply.apply(serverip, [true, response]);
            return reply.apply(serverip, [false, response]);
        });
